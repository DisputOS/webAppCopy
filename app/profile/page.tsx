          ...
          <>
            ...
            <div>
              ...
              <p className="text-white font-medium">
                {userData.plan ?? 'Free'}{' '}
                {userData.plan === 'Pro' && <span className="text-green-500 text-xs ml-2">✓ Pro Member</span>}
              </p>
            </div>
            ...
            <div className="pt-2 border-t border-gray-700">
              ...
              <p className="text-white font-medium">
                {userData.country_code ?? '—'} / {userData.language_code ?? '—'} / {userData.timezone ?? '—'}
              </p>
            </div>

            <div>
              <p className="text-sm text-gray-500">Subscription Status</p>
              <p className="text-white font-medium">{userData.subscription_status ?? 'None'}</p>
            </div>

            <div className="pt-2 border-t border-gray-700">
              <p className="text-sm text-gray-500">Stats</p>
              <p className="text-white font-medium">
                {userData.disputes_count ?? 0} disputes, {userData.proof_bundle_count ?? 0} proofs
              </p>
            </div>
            ...
            {userData.pdf_trust_rate && (
              <div>
                ...
                <p className="text-white font-medium">{(userData.pdf_trust_rate * 100).toFixed(1)}%</p>
              </div>
            )}

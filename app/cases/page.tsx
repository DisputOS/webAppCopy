...
// Responsive disputes list with safe text wrapping
// -----------------------------------------------------------------------------
'use client';
...
const NewDisputeModal = dynamic(() => import('@/components/NewDisputeModal'), {
  ssr: false,
});
  ...
  if (!session) {
    return (
      <>
        <Header />
        <main
          className="pt-[calc(56px+env(safe-area-inset-top))] h-[100svh] flex flex-col bg-gradient-to-b from-gray-900 via-gray-950 to-black text-white"
        >
  ...
  return (
    <>
      ...
      <main className="flex flex-col">
        {/* ── sticky title bar ─────────────────────────────────────── */}
        <div className="sticky top-0 z-20 flex items-center justify-between backdrop-blur px-6 py-4 bg-black/90">
              ...
              {disputes.map((d) => (
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    ...
                    {/* NEW badge */}
                    {d.created_at && Date.now() - new Date(d.created_at).getTime() < 86_400_000 && (
                      <span className="text-xs text-green-500 border border-green-500 rounded px-2 py-0.5 ml-2 shrink-0">
                        NEW
                      </span>
                    )}
                  </div>

                  <p className="text-sm text-gray-400 line-clamp-2 mb-4 break-words">
                    {d.description}
                  </p>
